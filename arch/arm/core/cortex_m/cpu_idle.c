...
void arch_cpu_idle(void)
{
	...
	/*
	 * The idle thread enters here after arch_zli_lock(), so only PRIMASK
	 * is set. Enable interrupts before WFI without touching the ARMv6-M
	 * zero-latency NVIC shadow state.
	 */
	__enable_irq();
	__ISB();
#else
